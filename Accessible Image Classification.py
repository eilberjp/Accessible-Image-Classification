...
def prep_model(params, verbosity):
    ...
    model = make_model(params['img_width'], params['img_height'],
                       params['color_mode'], params['num_classes'], verbosity)
    model.load_weights(params['weights'])
    compile_model(model, verbosity)
    return model
...
def train(directory, params, verbosity, vis):
    ...
    if vis:
        ...
        log_dir = Path(os.getcwd() + '/logs/' + log_dir_label + misc_params['time'])
        tensorboard = TensorBoard(log_dir=log_dir)
        callbacks.append(tensorboard)

    # do the training. Customize the ModelCheckpoint and EarlyStopping parameters freely
    if verbosity > 0:
        print('Retraining...')
    v_fit = 0
    if verbosity == 1:
...
def predict(directory, model, sort_class, params, verbosity, printout, rename):
    ...
    generator = gen_whole(directory, params['img_width'], params['img_height'],
                          params['batch_size'], params['color_mode'], verbosity)
    ...
    for label, index in generator.class_indices.items():
        if index == sort_class or label == sort_class:
            found = True
            if verbosity > 0:
...
def evaluate(directory, model, params, verbosity):
...
def visualize(directory, model, layers, cols, params, verbosity):
    ...
    j = len(os.listdir(directory))
    fig, axes2d = plt.subplots(nrows=int(np.ceil(j / 10)), ncols=cols, squeeze=False)
    axes = axes2d.flatten()
    ...
    for a, img_path in enumerate(os.listdir(directory)):  # go through each image in the directory
        ...
        if verbosity > 1:
            print('Visualizing image: ', img_path)
        ...
        for layer in layers:
            ...
            # overlay heat map with some transparency and a good colormap
...
train_params = {
    ...
    'num_classes': 3,  # how many different categories you want to train on
    'val_split': 0.25,  # proportion of training images to hold back for validation
    'color_mode': 'rgb',  # how to preprocess images to change color mode: "grayscale", "rgb", or "rgba"
...
misc_params = {
    'predict_sort_class': 0,  # class index or name to be used for sorting predicted images
    'vis_layers': [3, 6, 9, 10, 11, 12, 13],  # indices of the layers to be visualized
    'vis_images_per_row': 10,  # how many images per row to output during visualization
    'verbosity': 1,  # how much console output to generate (0=minimal, 1=some, 2=all)
    'time': ('/{}'.format(time()))  # just getting the time
}
...
train(directories['train'], train_params, misc_params['verbosity'], vis=True)

# create a fresh model and loads it with saved weights
loaded_model = prep_model(train_params, misc_params['verbosity'])

# output an evaluation of the model on the images in evaluation directory
evaluate(directories['eval'], loaded_model, train_params, misc_params['verbosity'])

# output the model's prediction for each image in prediction directory, by printing out to console and/or renaming files
predict(directories['pred'], loaded_model, misc_params['predict_sort_class'], train_params,
        misc_params['verbosity'], printout=True, rename=True)

# produce a plot of images in visualization directory with class activation mapping (attention visualization) overlay
visualize(directories['vis'], loaded_model, misc_params['vis_layers'], misc_params['vis_images_per_row'],
          train_params, misc_params['verbosity'])
